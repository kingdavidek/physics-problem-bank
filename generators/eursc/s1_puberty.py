...
_PM_POOLS = {
    "foundational": [
        _PM_MCQ("foundational", "phys", "A typical physical change at puberty is", _mcq_opts("growing shorter as the bones become denser", "growth and new body hair for many young people", "the skin becoming thinner and losing all its hair", "the heart shrinking to fit an adult chest"), "B", "Physical changes are general patterns, not a classroom inspection.", "Look for everyday body changes that often appear as a child grows toward adulthood, not changes that go backwards."),
        _PM_MCQ("foundational", "feel", "Emotional changes at puberty", _mcq_opts("never happen, because feelings are not part of biology", "can include stronger moods; this is discussed in third person, not as a confession", "happen only to people whose puberty starts early", "are the same for everyone and follow a fixed timetable"), "B", "Feelings can change; the quiz does not collect them.", "Moods can get stronger at this stage. Science describes that in general, and it does not depend on starting early or on a fixed timetable."),
        _PM_MCQ("foundational", "hormone", "Hormones are", _mcq_opts("nerve signals sent along the spinal cord", "chemical messengers that help trigger puberty changes", "nutrients that the body gets only from food", "types of blood cell that fight infection"), "B", "Hormones travel in the blood.", "They are chemicals that travel in the blood and carry messages that help trigger the changes of this stage."),
        _PM_MCQ("foundational", "vary", "The age when puberty starts", _mcq_opts("is exactly the same for everyone born in the same year", "varies; starting earlier or later than a friend can still be healthy", "is always later for people who play a lot of sport", "is fixed by how much a person ate as a baby"), "B", "Variation is normal.", "Friends the same age do not all change on the same timetable, and that can still be healthy."),
        _PM_MCQ("foundational", "not_ask", "This lesson must not", _mcq_opts("name the hormones involved in the changes", "ask a pupil to describe their own puberty or private body", "use fictional third-person examples of changes", "explain that the timing varies between people"), "B", "No personal disclosure.", "Science class can name facts. It must not collect private details about a pupil."),
        _PM_MCQ("foundational", "help", "If a fictional character is worried about puberty timing, a scientific classroom response is", _mcq_opts("ask the class to vote on whether the timing is normal", "suggest a teacher or qualified health professional, not a pupil survey", "compare them against a chart of classmates' ages", "tell them to wait and never mention it to anyone"), "B", "Signpost care.", "Worry about timing belongs with a trusted adult who can give proper advice, not with a class vote or a classmate chart."),
    ...
    "intermediate": [
        _PM_MCQ("intermediate", "both", "Sam and Lee are the same age. Sam has a deeper voice; Lee does not yet. Science says", _mcq_opts("Lee has a problem that needs fixing", "timing varies; neither person should be mocked", "Sam is further ahead, so Sam is healthier", "Lee's voice will never change now"), "B", "Variation is expected.", "Two people of the same age can be at different points in this stage. That is expected, not a contest."),
        _PM_MCQ("intermediate", "brain", "Mood changes at puberty are linked with", _mcq_opts("only diet, because sugar controls moods and hormones directly", "brain and hormone changes as well as social life; still not a reason to collect private diaries", "only growing taller, since height changes mood on its own", "only the weather, which switches hormone release on and off"), "B", "Biology plus life context; no diary harvest.", "Moods can link to chemicals in the blood, the brain, and everyday life. That is still not a reason to collect private diaries."),
        _PM_MCQ("intermediate", "mature", "Sexual maturity means", _mcq_opts("reaching adult height and stopping growing", "the body becoming able to produce mature gametes, at a time that varies", "turning eighteen and becoming a legal adult", "the point when all body hair has finished growing"), "B", "Biological capability, not a public identity test.", "This phrase is about whether the body can make mature sex cells, at a time that differs between people."),
        _PM_MCQ("intermediate", "compare", "Comparing who 'looks older' in class is", _mcq_opts("a fair way to estimate hormone levels", "not a scientific method and can be unkind", "a useful measurement of puberty stage", "acceptable as long as the results stay private"), "B", "No body ranking.", "Looking older is not a measurement. Science does not rank classmates by appearance."),
        _PM_MCQ("intermediate", "blood", "Hormones travel mainly in the", _mcq_opts("nerves", "blood", "air sacs", "bones"), "B", "Blood-borne messengers.", "These chemical messengers need a transport system that circulates around the body."),
        _PM_MCQ("intermediate", "signpost", "Personal medical questions about puberty belong with", _mcq_opts("the class group chat, so everyone can help", "a teacher or health professional, not this app's answer box", "this app's answer box, so it can be stored", "a popular influencer's advice video"), "B", "Signpost, do not store health stories.", "A personal medical worry belongs with a trusted adult or health professional, not a group chat or a video."),
        ...
        _PM_PICK("intermediate", "not_class", "Select the two actions that do not belong in a science class.", ["confess", "rank"], _CHANGE_BANK, 2, "No confession, no ranking.", "Choose the two actions that would turn the lesson into a private confession or a ranking of who looks more adult."),
    ],
    "difficult": [
        _PM_MCQ("difficult", "not_fail", "A person who starts puberty later than classmates", _mcq_opts("must have too few hormones to ever catch up", "may still be within a normal range; worry is for a professional, not a quiz", "will always end up shorter than the classmates", "should be given hormone tablets by the teacher"), "B", "Ranges, then signpost.", "Starting later than friends can still be within a normal range. Worry belongs with a professional, not a quiz score."),
        _PM_MCQ("difficult", "media", "Media images of 'ideal' teenage bodies", _mcq_opts("show the average stage for that age group", "are not a measurement of healthy puberty", "are chosen by doctors as a healthy target", "prove that early puberty is the healthy kind"), "B", "Images are not clinical data.", "Photos of an 'ideal' teenager are advertising or media, not a scientific sample of healthy change."),
        _PM_MCQ("difficult", "emotion_ok", "It is scientific to say moods can change at puberty. It is not scientific to", _mcq_opts("name the hormones linked to mood changes", "require pupils to submit private feelings as homework to this site", "use a fictional example of a moody teenager", "mention that the timing of mood changes varies"), "B", "No special-category mood files.", "Naming a general pattern is science. Demanding private feelings as homework is not."),
        _PM_MCQ("difficult", "gamete_ready", "Producing mature sperm or eggs is part of", _mcq_opts("the growth spurt, which happens at one fixed age", "sexual maturity, which arrives at different ages", "childhood, since gametes are made from birth", "adulthood only, starting exactly at age eighteen"), "B", "Link to 1.4.2 without asking who has started.", "Making mature sex cells is part of becoming sexually mature, and that happens at different ages."),
        _PM_MCQ("difficult", "help2", "Alex in a scenario wants private advice about body changes. The lesson answer is", _mcq_opts("type it into the quiz so it can give a diagnosis", "talk to a trusted adult or health professional; the quiz will not store that story", "ask classmates to compare their own changes with Alex's", "rely only on social media for the answer"), "B", "Signpost.", "Private worries about changing go to a trusted adult or health professional. This quiz will not store that story."),
...
_RA_POOLS = {
    "foundational": [
        _RA_MCQ("foundational", "egg", "The female gamete is the", _mcq_opts("sperm", "egg (ovum)", "ovary", "uterus"), "B", "Egg is the female gamete.", "A gamete is a sex cell. Which one is made in the ovaries?"),
        _RA_MCQ("foundational", "sperm", "The male gamete is the", _mcq_opts("egg", "sperm", "testis", "penis"), "B", "Sperm is the male gamete.", "A gamete is a sex cell. Which one is made in the testes?"),
        _RA_MCQ("foundational", "uterus", "The uterus is", _mcq_opts("the organ that releases eggs", "the organ where a fetus can develop", "the tube that carries urine", "the organ that produces sperm"), "B", "Uterus is the womb.", "This organ is the place where a fetus can grow. It is not the organ that releases eggs or the tube that carries urine."),
        _RA_MCQ("foundational", "ovary", "An ovary's job is to", _mcq_opts("produce sperm and store them", "release eggs and make some hormones", "hold the developing fetus", "carry urine out of the body"), "B", "Ovaries produce eggs.", "This organ releases eggs and also makes some chemical messengers. It does not hold the fetus or carry urine."),
        _RA_MCQ("foundational", "testis", "A testis can", _mcq_opts("release eggs", "produce sperm", "store urine", "hold a fetus"), "B", "Testes produce sperm.", "This organ produces the male sex cells. It does not release eggs."),
        _RA_MCQ("foundational", "fig", "<p>Which letter labels the uterus on this educational schematic?</p>" + str(organ_labels()), _mcq_opts("A", "B", "C", "none of the letters"), "B", "B is uterus. Schematic boxes only.", "Match the letter on the schematic to the organ where a fetus can develop."),
        ...
        _RA_NUM("foundational", "two_gametes", "The human gametes are the egg and the sperm. How many types is that?", 2, "Two types.", "Egg is one type and sperm is the other. Count how many types that is."),
    ...
    "intermediate": [
        _RA_MCQ("intermediate", "urinary", "The urethra can carry urine. In males it can also carry sperm at ejaculation. This means", _mcq_opts("urine and gametes are the same substance", "reproductive and urinary structures can share a pathway in males", "sperm are made in the bladder from urine", "urine and sperm pass along the tube at the same moment"), "B", "Shared pathway in males; still two jobs.", "One tube can have two jobs at different times. That does not make urine and sperm the same substance."),
        _RA_MCQ("intermediate", "fert", "Fertilisation is", _mcq_opts("the release of an egg from the ovary", "the joining of egg and sperm", "the shedding of the uterus lining", "the birth of the baby from the uterus"), "B", "Gametes join.", "This is the event when the two sex cells join."),
        _RA_MCQ("intermediate", "oviduct", "After release, an egg typically travels in the", _mcq_opts("vagina directly, without passing the uterus", "oviduct (Fallopian tube) toward the uterus", "bloodstream toward the heart", "urethra toward the bladder"), "B", "Oviduct to uterus.", "After leaving the ovary, the egg travels along a tube toward the uterus, not through the blood or the urethra."),
        _RA_MCQ("intermediate", "sperm_from", "Sperm are produced in the", _mcq_opts("ovaries", "testes", "penis", "bladder"), "B", "Testes.", "Where are male sex cells made?"),
        _RA_MCQ("intermediate", "cycle_point", "The menstrual cycle is", _mcq_opts("a one-off event that never repeats", "a repeating sequence involving hormones, ovulation and the uterus lining", "a single period that lasts for the whole month", "the time between fertilisation and the birth of a baby"), "B", "A cycle.", "A cycle repeats. Look for a repeating sequence that includes chemical messengers, egg release, and the uterus lining."),
        _RA_MCQ("intermediate", "fig_ovary", "<p>Which letter is an ovary?</p>" + str(organ_labels(title="A ovary")), _mcq_opts("B", "A", "C", "none of the letters"), "B", "A is ovary.", "Find the letter that marks the organ that releases eggs."),
    ...
    "difficult": [
        _RA_MCQ("difficult", "implants", "If fertilisation happens, the early embryo can implant in the", _mcq_opts("ovary", "uterus lining", "vagina", "bladder wall"), "B", "Implantation in the uterus.", "After the sex cells join, the early embryo attaches to the lining of the organ where a fetus can grow."),
        _RA_MCQ("difficult", "period_why", "A period happens when", _mcq_opts("an egg is released from the ovary", "the uterus lining is shed after an egg was not fertilised", "the uterus lining thickens ready for an egg", "a fertilised egg implants in the uterus lining"), "B", "Unfertilised cycle.", "If the egg is not fertilised, the built-up lining of the uterus is shed."),
        _RA_MCQ("difficult", "not_survey", "Teaching the menstrual cycle must not become", _mcq_opts("a labelled diagram of the reproductive organs", "a survey of who in the room has periods", "a list of the hormone names involved", "a third-person timeline of a typical cycle"), "B", "No period roll-call.", "A diagram and a sequence belong in class. A roll-call of who has started does not."),
        _RA_MCQ("difficult", "penis", "The penis can deliver sperm into the vagina. This is", _mcq_opts("a rumour with no basis in anatomy", "clinical reproductive anatomy, not a prompt for personal stories", "the stage when the egg is released from the ovary", "unrelated to how fertilisation can happen"), "B", "Clinical language; no confession.", "This is anatomy of how sperm can be delivered. It is not a prompt for personal stories."),
        _RA_MCQ("difficult", "fig_testis", "<p>Which letter is a testis?</p>" + str(organ_labels(title="C testis")), _mcq_opts("A", "C", "B", "none of the letters"), "B", "C is testis.", "Find the letter that marks the organ that produces sperm."),
        ...
        _RA_NUM("difficult", "one_egg", "Ovulation usually releases how many eggs in a typical cycle?", 1, "Usually one egg per cycle.", "A typical cycle releases one egg. That is the number to enter."),
...
_PS_POOLS = {
    "foundational": [
        _PS_MCQ("foundational", "sex", "Sexual intercourse can allow sperm to", _mcq_opts("enter the ovary and turn into an egg", "enter the vagina and swim toward an egg", "travel through the bloodstream to the uterus", "be stored in the uterus for many years"), "B", "Clinical mechanism, not a personal question.", "Think about the path sperm can take toward an egg. This is a mechanism, not a personal question."),
        _PS_MCQ("foundational", "preg", "Pregnancy means", _mcq_opts("the release of an egg from the ovary", "development after fertilisation, usually in the uterus", "the shedding of the uterus lining each month", "the moment a sperm and an egg join"), "B", "Development after fertilisation.", "After the sex cells join, development usually continues in the uterus."),
        _PS_MCQ("foundational", "contra", "Contraception is", _mcq_opts("a guarantee that pregnancy can never happen", "a set of methods that can reduce the chance of pregnancy", "a treatment that cures every kind of STI", "a method that only one partner in a couple can use"), "B", "Knowledge of methods; no 'which do you use'.", "These are methods that can lower the chance of pregnancy. The quiz does not ask what anyone uses."),
        _PS_MCQ("foundational", "sti", "An STI is", _mcq_opts("an infection that always shows obvious symptoms", "an infection that can spread through sexual contact", "a hormone that is released during puberty", "an illness that spreads only by coughing and sneezing"), "B", "Sexually transmitted infection.", "Some infections can spread through sexual contact."),
        _PS_MCQ("foundational", "consent", "Consent means", _mcq_opts("saying nothing, which counts as agreeing", "a clear, voluntary agreement that can be withdrawn", "a yes that cannot be taken back once it is given", "agreement that is assumed when two people are dating"), "B", "Yes can be withdrawn.", "A real yes is clear, freely given, and can be taken back."),
        _PS_MCQ("foundational", "orient", "Sexual orientation is the idea that", _mcq_opts("everyone is attracted to the same sex as their friends are", "people can be attracted to different sexes; nobody is asked which applies to them", "attraction is decided by a hormone test taken at puberty", "science has no facts about attraction, so it is never taught"), "B", "Fact without disclosure.", "People can be attracted to different sexes. The lesson states that as a fact, without asking which applies to a pupil."),
    ...
    "intermediate": [
        _PS_MCQ("intermediate", "fetus", "A fetus is", _mcq_opts("the egg cell before it has been fertilised", "the developing offspring in the uterus after the early embryo stage", "the lining of the uterus that is shed each cycle", "the organ that passes food and oxygen to the baby"), "B", "S1 wording: developing in the uterus.", "After the early embryo stage, this is the name for the offspring developing in the uterus."),
        _PS_MCQ("intermediate", "birth", "Birth is usually", _mcq_opts("the moment when the egg and the sperm join", "the baby leaving the uterus, often through the vagina", "the fetus moving from the ovary into the uterus", "the egg being released from the ovary"), "B", "Clinical, not graphic.", "The baby leaves the uterus, often through the vagina."),
        _PS_MCQ("intermediate", "condom", "A condom is discussed here as", _mcq_opts("a hormone method that stops eggs being released", "a barrier method that can reduce pregnancy chance and some STI risk", "a method that guarantees no pregnancy and no STI ever", "a method that protects against STIs but not pregnancy"), "B", "Barrier method knowledge.", "A barrier method can lower pregnancy chance and some infection risk."),
        _PS_MCQ("intermediate", "test", "If a scenario character might have an STI, the scientific next step is", _mcq_opts("wait to see whether the symptoms disappear on their own", "qualified health advice and testing; this app does not diagnose", "search online for a diagnosis and treat it at home", "assume there is no infection because there is no pain"), "B", "Signpost health care.", "Possible infection needs qualified testing and advice — not waiting or a home guess."),
        _PS_MCQ("intermediate", "media", "Media can show unrealistic sex or relationships. A scientific habit is to", _mcq_opts("treat a popular video as evidence of what is normal", "compare claims with curriculum facts and consent rules", "assume anything that was filmed must be accurate", "trust a clip more when it has a lot of views"), "B", "Critique media; no viewing-history harvest.", "Check videos against curriculum facts and consent rules, rather than treating any clip as evidence."),
        _PS_MCQ("intermediate", "no", "Jordan hears 'no' and continues. That is", _mcq_opts("fine if dating", "not consent", "still consent", "a grey area"), "B", "No means no.", "If someone says no, continuing is not agreement."),
    ...
    "difficult": [
        _PS_MCQ("difficult", "identity", "Gender identity and sexual orientation", _mcq_opts("are the same thing, and are decided by a hormone test", "can differ between people; classwork uses general facts, never a roll-call", "are fixed for everyone by the sex organs they were born with", "cannot be discussed, because science has no facts about them"), "B", "Curriculum-faithful and non-inquisitive.", "These ideas can differ between people. Classwork uses general facts, never a roll-call."),
        _PS_MCQ("difficult", "comm", "Healthy communication in a relationship includes", _mcq_opts("ignoring a withdrawn yes because it was given once", "listening and respecting a no, in third-person scenarios", "assuming that silence means the other person agrees", "expecting a yes from anyone who is in a relationship"), "B", "Communication and consent.", "Healthy communication includes listening and respecting a no."),
        _PS_MCQ("difficult", "limit", "Contraception can fail. That is why", _mcq_opts("no method is worth using, since none of them ever work", "methods reduce chance; they are not a magic shield, and personal choices belong with qualified advice", "using two methods at once makes pregnancy completely impossible", "only hormone methods can fail; barrier methods never do"), "B", "Reduce chance, not certainty; signpost.", "Methods can fail, so they reduce chance rather than giving a magic shield."),
        _PS_MCQ("difficult", "preg_help", "A fictional character is pregnant and scared. The classroom answer is", _mcq_opts("to wait a few months and see what happens", "a trusted adult or health professional; do not harvest private stories here", "a home test and advice from friends only", "to post the situation online for other opinions"), "B", "Signpost.", "A scared fictional character needs a trusted adult or health professional, not a quiz diagnosis."),
        _PS_MCQ("difficult", "sti2", "Some STIs can be present with few symptoms. That is why", _mcq_opts("no symptoms means there is no infection to pass on", "qualified testing matters; nobody is asked who has been tested", "testing is only needed once symptoms have appeared", "a person can always tell from how they feel whether they are infected"), "B", "Asymptomatic possibility; no test-history survey.", "Few symptoms does not mean there is no infection. Qualified testing still matters; nobody is asked who has been tested."),
