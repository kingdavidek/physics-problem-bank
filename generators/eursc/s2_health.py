...
_HL_POOLS = {
    ...
    "intermediate": [
        ...
        _HL_MCQ("intermediate", "respect", "Respectful relationships means", _mcq_opts("the quiz storing who likes whom", "communication and consent ideas without asking for private stories", "ranking popularity", "ignoring a classmate in distress as a health plan"), "B", "Curriculum idea, not a social graph.", "This is about communication and consent as ideas, not storing who likes whom."),
...
_ID_POOLS = {
    "foundational": [
        _ID_MCQ("foundational", "bact", "Bacteria are", _mcq_opts("a unit of time", "living cells that can act as pathogens", "the same as a virus always", "a class attendance list"), "B", "Living cells.", "These are living cells that can cause disease — not a unit of time or an attendance list."),
    ...
    "intermediate": [
        ...
        _ID_MCQ("intermediate", "abx", "Antibiotics", _mcq_opts("treat all viruses", "can treat some bacterial infections but not viruses, and resistance matters", "are a food group", "must be listed from home cabinets in the quiz"), "B", "Not for viruses; resistance.", "These medicines can help some bacterial infections. They do not work on viruses, and resistance matters."),
        ...
        _ID_MCQ("intermediate", "epi", "Epidemiology uses", _mcq_opts("secret rumours only", "counts and patterns that can be checked, not a pupil's file", "a ranking of bodies", "Moon phases as the only cause"), "B", "Public counts.", "This subject uses counts and patterns that can be checked, not a pupil's file."),
    ...
    "difficult": [
        ...
        _ID_MCQ("difficult", "bar_c", "<p>Which day has the most cases?</p>" + str(outbreak_bars(title="Most cases")), _mcq_opts("A", "C", "B", "a named pupil"), "B", "C is 8 cases.", "Compare the three day-bars and pick the letter with the highest case count."),
...
_NI_POOLS = {
    ...
    "difficult": [
        ...
        _NI_MCQ("difficult", "two_n", "Infectious and non-infectious: how many kinds of disease is that?", _mcq_opts("one", "two", "eight", "zero"), "B", "Two.", "Count the top-level kinds: infectious versus not."),
...
_DA_POOLS = {
    "foundational": [
        _DA_MCQ("foundational", "pleasure", "Ordinary pleasure is", _mcq_opts("the same as dependence always", "possible without being dependence", "a reason to collect use lists", "a vaccine"), "B", "Pleasure is not automatically addiction.", "Enjoyment can be part of life without automatically being dependence."),
    ...
    "difficult": [
        ...
        _DA_MCQ("difficult", "help2", "Support routes are", _mcq_opts("this app as a clinic", "trusted adults and qualified services; no treatment record is stored here", "a popularity poll", "hiding all signposts"), "B", "Signpost only.", "Support is trusted adults and qualified services. No treatment record is stored here."),
        ...
        _DA_MCQ("difficult", "two_again", "Substance and behavioural: how many kinds of dependence is that?", _mcq_opts("one", "two", "fourteen", "zero"), "B", "Two.", "Count the two kinds: chemical versus repeated action."),
...
_TB_POOLS = {
    "foundational": [
        ...
        _TB_MCQ("foundational", "prevent", "Prevention means", _mcq_opts("forcing a confession in the quiz", "reducing uptake using public health ideas, not a use survey", "ignoring nicotine", "ranking who looks sporty"), "B", "Reduce uptake, no confession.", "The aim is to reduce uptake using public-health ideas, not to run a confession."),
        _TB_MCQ("foundational", "vape", "Vaping is", _mcq_opts("proven harmless for everyone", "still uncertain in important ways; nicotine can still addict", "a fruit", "a demand to list devices"), "B", "Uncertainty plus nicotine.", "The scientific position is uncertainty plus the same addictive chemical — not 'harmless for everyone'."),
    ...
    "intermediate": [
        ...
        _TB_MCQ("intermediate", "bar_more", "<p>In this sketch, which bar is the largest count?</p>" + str(outbreak_bars(title="Largest count")), _mcq_opts("A", "C", "B", "a named smoker"), "B", "C is the tall bar.", "Compare the bars and pick the letter with the largest count in the sketch."),
    ...
    "difficult": [
        ...
        _TB_MCQ("difficult", "both_n", "Tobacco smoke and many vapes can both involve", _mcq_opts("only vitamins", "nicotine and therefore addiction risk", "a requirement to confess", "a geocentric vote"), "B", "Nicotine overlap.", "Smoke and many vapes can share the same addictive chemical, so addiction risk still applies."),
