"""A-level physics — magnetism (fields, motor effect, induction, etc.)."""
import random
import math
...
_MAG_FOUND_POOL = [
    ...
    _mag_found_field_definition,
    _mag_found_fleming_left,
    _mag_found_flux_definition,
    _mag_found_induced_emf_condition,
]
...
_MAG_INTER_POOL = [
    ...
    _mag_inter_force_calc,
    _mag_inter_charge_force,
    _mag_inter_flux_linkage,
    _mag_inter_transformer,
]
...
_MAG_EXAM_DIFF_POOL = [
    ...
    mag_exam_motor_effect,
    mag_exam_faraday_lenz,
]
...
_MAG_DIFF_POOL = [
    ...
    _mag_diff_cyclotron,
    _mag_diff_faraday_explain,
    _mag_diff_lenz_application,
    _mag_diff_halbach_array,
]

_MAG_DIFFICULT_POOL = _MAG_DIFF_POOL + _MAG_EXAM_DIFF_POOL

_MAG_POOL_BY_TIER = {
    'foundational': _MAG_FOUND_POOL,
    'intermediate': _MAG_INTER_POOL,
    'difficult': _MAG_DIFFICULT_POOL,
}


def _mag_variant_by_name(name):
    for pool in (_MAG_FOUND_POOL, _MAG_INTER_POOL, _MAG_DIFFICULT_POOL):
        for fn in pool:
            if fn.__name__ == name:
                return fn
    return None
...
def alevel_physics_magnetism(difficulty, mode, variant_name=None):
    if mode == 'mcq':
        q_mcq, s_mcq, hint_mcq, marks_mcq, options_mcq, correct_mcq = mag_mcq()
        return make_problem(
    ...
    if variant_name:
        variant = _mag_variant_by_name(variant_name)
        if variant is None:
            variant = random.choice(_MAG_DIFFICULT_POOL)
        ...
        difficulty = tier
        variant = random.choice(_MAG_POOL_BY_TIER[tier])
    else:
        pool = _MAG_POOL_BY_TIER.get(difficulty, _MAG_FOUND_POOL)
