"""Smoke checks for energy situational multi-step pilot content."""
import os
import random
import re
import sys
...
def test_energy_sms_variants_are_grader_ready():
    ...
    for difficulty in DIFFS:
        variants = eursc_science_energy_variants(
        ...
        for fn in variants:
            ...
            assert not POWER_CALC_RE.search(blob), fn.__name__
            assert not VIR_CALC_RE.search(blob), fn.__name__
            assert "fictional" in blob.lower(), fn.__name__
            if "<svg" in str(problem.get("question") or ""):
                saw_sankey = True
            via_generate = eursc_science_energy(
...
def test_energy_sms_solutions_link_parts():
...
def test_energy_sms_randomization():
    fn = next(
        f
        for f in eursc_science_energy_variants(
            "foundational", SITUATIONAL_MULTI_STEP_MODE
        )
        if f.__name__ == "energy_foundational_sms_appliance_useful_public"
    )
    stems = set()
...
def test_energy_sms_api_generate():
    ...
    assert problem["topic"] == "energy"
    assert problem.get("answer_type") == "number_fields"


def main():
    test_energy_registers_situational_mode()
