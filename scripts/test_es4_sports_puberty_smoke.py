"""ES4 — S1 Sports + Puberty (1.3.1–1.4.3) smoke.

Run: python scripts/test_es4_sports_puberty_smoke.py
"""
import os
import re
import sys
...
QUIZ_SLUGS = ('movement', 'sport_health', 'pregnancy_sexual_health')
SVG_SLUGS = {
    'movement': 'Distance–time',
    'forces_sport': 'Two objects pushing',
    'breathing': 'Circulation schematic',
    'sport_health': 'Antagonistic muscles',
    'reproductive_anatomy': 'Educational labels',
}
DISCLOSE_RE = re.compile(
    ...
    r'have you been pregnant|are you pregnant|describe your body)\b',
    re.I,
)
...
def register(client, suffix):
    r = client.post(
        ...
        json={
            'email': f'es4_{suffix}@example.com',
            'handle': f'es4_{suffix}',
...
def test_manifest_registry_templates():
    ...
    for ref in NEW_REFS:
        ...
        unit_code, unit_name = UNIT_BY_REF[ref]

        assert cfg['name'] == module['name']
        assert cfg['order'] == module['order']
        assert cfg['year'] == 's1'
        assert cfg['unit_code'] == unit_code
        assert cfg['unit_name'] == unit_name
    ...
    assert science['pregnancy_sexual_health']['order'] == 18
...
def test_practice_and_qotd_stay_closed():
    ...
    with app.test_client() as client:
        ...
        assert 'data-topic="movement"' in chunk
...
def test_lesson_routes_topics_search():
    with app.test_client() as client:
        ...
        assert '/topic/eursc/science/movement' in topics_html
        ...
        assert 'movement' in slugs
        assert 'pregnancy_sexual_health' in slugs
...
def main():
    ...
    test_mixed_quiz_api()
    print('ES4 Sports + Puberty lesson smoke tests passed.')
