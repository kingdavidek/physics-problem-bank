"""ES5 — S2 Universe (2.1.1–2.1.4) + light IBL support smoke.

Run: python scripts/test_es5_universe_smoke.py
"""
import os
import re
import sys
...
NEW_REFS = ('2.1.1', '2.1.2', '2.1.3', '2.1.4')
QUIZ_SLUGS = ('solar_system', 'light_telescopes', 'atoms_molecules')
SVG_SLUGS = {
    'solar_system': 'Sun, Earth and Moon',
    'light_telescopes': 'Reflection at a mirror',
    'atoms_molecules': 'Atom, molecule',
}
IBL_LINK_SLUGS = ('solar_system', 'light_telescopes')
...
def register(client, suffix):
    r = client.post(
        ...
        json={
            'email': f'es5_{suffix}@example.com',
            'handle': f'es5_{suffix}',
...
def test_manifest_registry_templates():
    ...
    for ref in NEW_REFS:
        ...
        assert cfg['year'] == 's2'
        assert cfg['unit_code'] == '2.1'
        assert cfg['unit_name'] == 'Universe'
    ...
    assert 's2_light' not in science
...
def test_practice_and_qotd_stay_closed():
    ...
    with app.test_client() as client:
        ...
        assert 'data-topic="solar_system"' in chunk
...
def test_ibl_page():
    ...
    with app.test_client() as client:
        ok = client.get('/ibl/eursc/science/s2_light')
...
def main():
    ...
    test_ibl_page()
    test_mixed_quiz_api()
    print('ES5 Universe lessons + IBL smoke tests passed.')
