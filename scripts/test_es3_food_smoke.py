"""ES3 — S1 Food (1.2.1–1.2.8) + second IBL support smoke.

Run: python scripts/test_es3_food_smoke.py
"""
import os
import re
import sys
...
QUIZ_SLUGS = ('food_formulas', 'nutrition', 'healthy_meal_project')
DISCLOSE_RE = re.compile(
    r'\b(your diet|have you ever|tell us about your|describe your eating|'
    r'are you allergic|what are you allergic)\b',
    re.I,
)
...
def register(client, suffix):
    r = client.post(
        ...
        json={
            'email': f'es3_{suffix}@example.com',
            'handle': f'es3_{suffix}',
...
def test_manifest_registry_templates():
    ...
    for ref in NEW_REFS:
        ...
        assert cfg['year'] == 's1'
        assert cfg['unit_code'] == '1.2'
        assert cfg['unit_name'] == 'Food'
...
def test_bank_size_and_formats():
    for ref in NEW_REFS:
        ...
        if slug in ('water_substances', 'cooking_salt', 'nutrition', 'healthy_meal_project'):
            assert 'number' in kinds
        if slug == 'water_substances':
            assert any(
                '<svg' in str(fn().get('question'))
                for fn in cfg['variants_func']('foundational', 'lesson')
            )
...
def test_practice_and_qotd_stay_closed():
    ...
    with app.test_client() as client:
        ...
        assert 'Molecules of Life' not in body or 'data-launch-gcse-only="1"' in html
...
def test_lesson_routes_topics_search():
    with app.test_client() as client:
        for ref in NEW_REFS:
            ...
            assert ref in html
            if slug == 'water_substances':
                assert 'Particle arrangements' in html or 'packed' in html
            if slug == 'cooking_acid':
                assert 'pH scale' in html or 'alkali' in html
...
def test_ibl_page():
    ...
    with app.test_client() as client:
        ok = client.get('/ibl/eursc/science/s1_food')
...
def main():
    ...
    test_ibl_page()
    test_mixed_quiz_api()
    print('ES3 Food lessons + IBL smoke tests passed.')
