...
| **Mobile polish (app-like PWA)** | **M0–M3 shipped** (foundation, practice UX, app chrome, lessons/diagrams/Python honesty). **M4** still planned — see `docs/MOBILE.md`. **M5–M7** HTTPS → TWA → Play Android after M4 |
...
1. **Mobile polish M4** per `docs/MOBILE.md` — PWA polish, device QA matrix, status update (M0–M3 are done).
