...
**E7 (mascot motion + onboarding) Phase 0 started (2026-09-22).** Decisions frozen in `docs/MASCOT_MOTION_AND_ONBOARDING.md` §2 #1-13 (rig-only, WAAPI, no library, cosmetics not a second character, CSS budget raised). `scripts/test_zorp_motion_smoke.py` added as the Phase 0 skeleton (bans Lottie/jsdelivr/unpkg repo-wide; will check motion.css's keyframe/reduced-motion pairing and 8KB cap once Phase 1 creates that file). Next: Phase 1 (rig + runtime + idle).
