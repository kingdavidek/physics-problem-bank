...
| **Phase U (UI redesign)** | **U0–U5 shipped.** U7.1–U7.8 shipped. **U6.1–U6.4 shipped** (vocabulary, mensuration, migrator, applied to 38 lessons). Next: **U6.5** only after leftovers no longer need `div[style*="max-width:860px"]` compensation (wrappers are now `.lesson-shell`; some inner `style=""` remain). Then U6.6 radioactivity, U6.7 stubs, U6.8 stray `.py`, then U8. Spec: `docs/UI_REDESIGN.md` §9. Cache: `pb-v55`. |
