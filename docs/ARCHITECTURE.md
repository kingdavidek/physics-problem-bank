...
| **PWA** | Service worker + manifest | Offline shell; API always network-only. **M0–M2** shipped (`base.html` + nav/search/notif sheets, `--kb-inset` / visualViewport, `:focus-visible`). M3–M4 still planned — `docs/MOBILE.md` |
