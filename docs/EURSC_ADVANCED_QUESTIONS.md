...
#### Batch 3.3 — Unit 2.3 Senses (2026-09-13)

| Field | Record |
|---|---|
| **Topics** | `vision`, `hearing`, `touch`, `smell`, `taste`, `proprioception_balance`, `interoception`, `nonhuman_senses` |
| **Enabled cells** | Exactly the matrix: `vision`/`hearing` MS (I/D) + SMS (F/I/D); `touch`/`taste`/`proprioception_balance` MS (I/D) + SMS (I/D); `smell` and `interoception` SMS (I/D) only — their MS rows stay **—** and the slugs advertise `standard, situational_multi_step` only; `nonhuman_senses` full matrix. 96 new variant functions in `generators/eursc/s2_unit23_senses_advanced.py`, three named blueprints per enabled cell, pack tuples (≥3) wherever a blueprint randomises. Every excluded cell is an empty list and fails closed at the variants, generate and API layers. |
| **Safeguarding gate** | Written into the module docstring and enforced by the smoke: third-person fictional cases, textbook/public tables, supplied aggregate datasets (two-point thresholds, nose-clip panels, eyes-open/closed stance counts) or fictional lab/field models only; no stem asks a pupil to test, describe, rank or record their own senses, glasses, hearing aids, balance, hunger, heartbeat or mood, and none asks classmates to test each other (`DISCLOSE_RE` plus a Unit 2.3 `SENSES_DISCLOSE_RE`); interoception cases run ambiguous signal → alternative interpretations → trusted-adult signpost and never diagnose; clinical references (optician, audiology, balance and neurology clinics) stay with fictional characters. |
| **Graders** | Existing only: `number_fields` with `number`, `mcq`, `keyword`, `order`, `pick`. Echo/sonar distance uses \(d = vt/2\) with supplied speeds; no new physics is claimed. |
| **Smokes** | New `scripts/test_es_s2_unit23_senses_advanced_smoke.py` (per-slug capability tuples, every excluded cell empty and clamped, grader-ready ×6 renders, same-variant pin, pack-once check, lesson/standard snapshots, API generate incl. six excluded-cell fallbacks). Updated `test_es_advanced_pilot_exit_smoke.py` (`S2_UNIT23_ENABLED`; fail-closed API fixture moved from `vision` MS to `magnetism` MS). Regressions green: `test_es7_senses_smoke.py`, `test_es10_whole_suite_smoke.py`, `test_es_practice_slots_smoke.py`, `test_es_advanced_downstream_smoke.py`, `test_es_advanced_mode_plumbing.py`. |
| **Fixture note** | `magnetism` MS (pilot-exit) and `electric_current` (pilot-exit SMS + plumbing) are temporary "no advanced modes" fixtures that the S3 wave must move. Permanent fail-closed cells: `smell` MS, `interoception` MS, `reproductive_anatomy` SMS. |
| **Next** | **S2 wave complete.** S3 wave (`force_work_machines` … `ecology_field_project`): add explicit content tests rejecting power calculations and \(V=IR\)/resistance; project topics grade planning and evidence only; environmental scenarios use supplied public/aggregate data. Then the whole-matrix audit. |
